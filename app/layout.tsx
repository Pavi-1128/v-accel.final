...
const StarsCanvas = dynamic(
  () =>
    import("@/components/main/star-background").then((mod) => ({
...
const WhatsAppLogo = dynamic(
  () =>
    import("@/components/main/whatsapp-logo").then((mod) => ({
      default: mod.WhatsAppLogo,
    })),
  {
    ssr: false,
    loading: () => null,
...
export default function RootLayout({ children }: PropsWithChildren) {
  return (
    <html lang="en">
      ...
      <body
        className={`${inter.className} min-h-screen relative overflow-x-hidden bg-gradient-to-b from-[#030014] via-[#0a011a] to-[#060014]`}
      >
